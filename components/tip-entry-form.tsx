...
export function TipEntryForm() {
  ...
  React.useEffect(() => {
    async function loadData() {
      ...
      const currentSession = searchParams.get("session") || `session_${Date.now()}`;
    ...
    <div className="mx-auto w-full max-w-md space-y-6 p-4">
      <div className="text-center">
        ...
        <p className="text-muted-foreground">
          {step === "bill" ? "Powered by Tink Protocol" : "Review Your Tip"}
        </p>
      </div>
      ...
      <div className="flex items-center justify-center space-x-4">
        <div 
          className={cn("flex items-center gap-2 cursor-pointer", step === 'bill' ? "text-foreground" : "text-muted-foreground")}
          onClick={() => setStep('bill')}
        >
          <div className={cn("h-8 w-8 rounded-full flex items-center justify-center border text-sm font-medium transition-colors", step === 'bill' ? "border-primary bg-primary/10 text-primary" : "border-border bg-secondary")}>1</div>
          <span className="text-sm font-medium">Bill</span>
        </div>
        <div className="h-px w-12 bg-border" />
        <div className={cn("flex items-center gap-2", step === 'tip' ? "text-foreground" : "text-muted-foreground")}>
          <div className={cn("h-8 w-8 rounded-full flex items-center justify-center border text-sm font-medium transition-colors", step === 'tip' ? "border-primary bg-primary/10 text-primary" : "border-border bg-secondary")}>2</div>
